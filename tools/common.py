...
def save_analysis(
    analysis: str, session_id: str, run_id: str, complete: bool = False, account_id: str = None, *args, **kwargs
) -> Dict[str, str]:
    """
    ...
    try:
        log_db = LogDB()
        logger.info(
            f"[analysis] saving to DB session={session_id} run_id={run_id} analysis={insights_text[:100]}... complete={complete}"
        )
        log_db.add_insight(
            session_id=session_id,
            run_id=run_id,
